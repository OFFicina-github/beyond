...
function nectar_main_styles() {
		...
		// override default WPBakery front style with optimized version.
		if ( defined('SALIENT_VC_ACTIVE') ) {
			wp_deregister_style( 'js_composer_front' );
		 	wp_register_style( 'js_composer_front', $nectar_get_template_directory_uri . '/css/'.$src_dir.'/plugins/js_composer.css', array(), $nectar_theme_version );
			if ( apply_filters('salient_enable_core_wpbakery_post_grid', false) === true ) {
				wp_enqueue_style( 'js_composer_post-grid-icons', $nectar_get_template_directory_uri . '/css/'.$src_dir.'/plugins/js_composer_post-grid.css', array(), $nectar_theme_version );
			}
		}
	...
	$salient_brands = nectar_get_brands_social_list();
	foreach ( $salient_brands as $brand ) {
		if( isset($nectar_options[$brand.'-url']) && !empty($nectar_options[$brand.'-url']) ||
			isset($nectar_options['use-'.$brand.'-icon-header']) && !empty($nectar_options['use-'.$brand.'-icon-header']) ||
			isset($nectar_options['use-'.$brand.'-icon']) && !empty($nectar_options['use-'.$brand.'-icon']) ) {
			wp_enqueue_style('nectar-brands');
			break;
		}
	}
