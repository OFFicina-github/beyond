			...
			<div class="col span_9 col_last header-beyond">
				...
				if (has_action('nectar_hook_mobile_header_menu_items')) { ?>
					<div class="nectar-mobile-only mobile-header">
						<div class="inner"><?php nectar_hook_mobile_header_menu_items(); ?></div>
					</div>
				<?php }
				...
				if ($nectar_header_options['header_search'] != 'false') {
					...
					<a class="mobile-search" href="#searchbox"><span class="nectar-icon icon-salient-search"
							...
							class="screen-reader-text"><?php echo esc_html__('search', 'salient'); ?></span></a>
					<?php
				}
				...
				if ($nectar_header_options['user_account_btn'] != 'false') {
					...
					<a class="mobile-user-account"
						href="<?php echo esc_url($nectar_header_options['user_account_btn_url']); ?>"><span
							class="normal icon-salient-m-user" aria-hidden="true"></span><span
							class="screen-reader-text"><?php echo esc_html__('account', 'salient'); ?></span></a>
					<?php
				}
				...
				if (!empty($nectar_options['enable-cart']) && $nectar_options['enable-cart'] === '1') {
					...
					if ($woocommerce) {
						...
						<a id="mobile-cart-link" aria-label="<?php echo esc_attr__('Cart', 'salient'); ?>"
							data-cart-style="<?php echo esc_attr($nav_cart_style); ?>"
							href="<?php echo esc_url(wc_get_cart_url()); ?>"><i class="icon-salient-cart"></i>
							<div class="cart-wrap"><span><?php echo esc_html($woocommerce->cart->cart_contents_count); ?>
								</span></div>
						</a>
						<?php
					}
				...
				if ($nectar_header_options['side_widget_area'] === '1' || $nectar_header_options['side_widget_class'] === 'simple') {
					...
					<div class="slide-out-widget-area-toggle mobile-icon <?php echo esc_attr($nectar_header_options['side_widget_class']); ?>"
						data-custom-color="<?php echo esc_attr($nectar_header_options['ocm_menu_btn_color']); ?>"
						data-icon-animation="simple-transform">
						<div> <a href="<?php echo $ocm_anchor; ?>" role="button"
								aria-label="<?php echo esc_attr__('Navigation Menu', 'salient'); ?>" aria-expanded="false"
								class="<?php echo 'closed' . esc_attr($menu_label_class); ?>">
								<?php if (true === $menu_label) {
									echo '<i class="label">' . esc_html__('Menu', 'salient') . '</i>';
								} else {
									echo '<span class="screen-reader-text">' . esc_html__('Menu', 'salient') . '</span>';
				...
				<nav aria-label="<?php echo esc_html__('Main Menu', 'salient'); ?>">
					...
					if (in_array($nectar_header_format, array('centered-logo-between-menu-alt'))) {
						...
						<ul class="sf-menu">
							...
							if (
								!empty($nectar_options['enable_social_in_header']) &&
					...
					if (
						false === in_array($nectar_header_format, array('menu-left-aligned', 'centered-logo-between-menu-alt')) &&
